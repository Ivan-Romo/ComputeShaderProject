...
public class PixelSimulatorGPU : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        int smoothKernel = computeShader.FindKernel("SmoothTrail");
        computeShader.SetTexture(smoothKernel, "Result", texture);
        computeShader.SetInt("width", width);
        computeShader.SetInt("height", height);
        computeShader.Dispatch(smoothKernel, Mathf.CeilToInt(width / 8f), Mathf.CeilToInt(height / 8f), 1);
